...
// node_modules/vader-sentiment/src/vader_lexicon.js
var vader_lexicon_exports = {};
__export(vader_lexicon_exports, {
  lexicon: () => lexicon
});
var lexicon;
var init_vader_lexicon = __esm({
  "node_modules/vader-sentiment/src/vader_lexicon.js"() {
    lexicon = {
      "$:": -1.5,
      "%)": -0.4,
...
// node_modules/vader-sentiment/src/vaderSentiment.js
var LEXICON = (init_vader_lexicon(), __toCommonJS(vader_lexicon_exports)).lexicon;
var B_INCR = 0.293;
var B_DECR = -0.293;
var C_INCR = 0.733;
var N_SCALAR = -0.74;
var REGEX_REMOVE_PUNCTUATION = new RegExp(/[!"#$%&'()*+,-./:;<=>?@[\\\]^_`{|}~]/g);
var PUNC_LIST = [
  ".",
  "!",
  "?",
...
var negated = (input_words, include_nt = true) => {
  let neg_words = [];
  neg_words.push.apply(neg_words, NEGATE);
  for (let i2 = 0; i2 < neg_words.length; i2++) {
    if (input_words.indexOf(neg_words[i2]) >= 0) {
      return true;
    }
  }
  if (include_nt === true) {
    for (let i2 = 0; i2 < input_words.length; i2++) {
      if (input_words[i2].indexOf("n't") >= 0) {
        return true;
      }
    }
  }
...
var normalize = (score, alpha = 15) => {
  const norm_score = score / Math.sqrt(score * score + alpha);
  if (norm_score < -1) {
    return -1;
  } else if (norm_score > 1) {
    return 1;
  } else {
    return norm_score;
  }
};
var allcap_differential = (words) => {
  let allcap_words = 0;
...
var scalar_inc_dec = (word, valence, is_cap_diff) => {
  let scalar = 0;
...
var SentimentIntensityAnalyzer = class _SentimentIntensityAnalyzer {
  /**
  ...
  */
  static polarity_scores(text) {
    ...
    for (let i = 0; i < words_and_emoticons.length; i++) {
      ...
      sentiments = _SentimentIntensityAnalyzer.sentiment_valence(
        valence,
        sentiText,
        item,
        i,
    ...
    sentiments = _SentimentIntensityAnalyzer.but_check(words_and_emoticons, sentiments);
    const valence_dict = _SentimentIntensityAnalyzer.score_valence(sentiments, text);
    return valence_dict;
  }
  ...
  static punctuation_emphasis(sum_s, text) {
    const ep_amplifier = _SentimentIntensityAnalyzer.amplify_ep(text);
    const qm_amplifier = _SentimentIntensityAnalyzer.amplify_qm(text);
  ...
  static score_valence(sentiments, text) {
    if (sentiments && sentiments.length > 0) {
      ...
      const punct_emph_amplifier = _SentimentIntensityAnalyzer.punctuation_emphasis(sum_s, text);
      if (sum_s > 0) {
      ...
      let scores = _SentimentIntensityAnalyzer.sift_sentiment_scores(sentiments);
...
// node_modules/vader-sentiment/src/index.js
var index_default = SentimentIntensityAnalyzer;
export {
